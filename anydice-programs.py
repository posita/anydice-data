    ...
    *,
    ...
    for i, (program_id, program) in enumerate(rows, start=1):
        ...
        if debug:
            ...
            print(
                f"debug: [{i:>{idx_width}}/{total}] done "
                f"program_id={hex_id} in {elapsed:.2f}s",
                file=sys.stderr,
            )
...
def _verify_worker(
    ...
    to their rows) and the elapsed wall-clock time spent in `_classify`
    (so the parent can report per-task timing under `--debug`). Module-level
    so it pickles cleanly under `forkserver`.
    """
    program_id, program, output_json, timeout_s = args
    t_start = time.monotonic()
    bucket, detail = _classify(program, output_json, timeout_s=timeout_s)
    elapsed = time.monotonic() - t_start
    return program_id, bucket, detail, elapsed
    ...
    with ctx.Pool(
        ...
        try:
            for i, (program_id, ar) in enumerate(async_results, start=1):
                ...
                # `t_collect_start` brackets the parent-side wait so we can
                # report an elapsed time even when the worker raises (and thus
                # never reports its own measured elapsed via the return tuple).
                # With `--isolated-workers 1` the parent-side measurement
                # approximates the actual task duration; with more workers it
                # overstates for later tasks (which may have been running in
                # parallel with earlier ones).
                ...
                if elapsed is None:
                    elapsed = time.monotonic() - t_collect_start
                if debug:
                    print(
                        f"debug: [{i:>{idx_width}}/{total}] done "
                        f"program_id={program_id:x} bucket={bucket} "
                        f"in {elapsed:.2f}s",
                        file=sys.stderr,
                    )
