$(function(){
	...
	function CheckDonation(){
		...
		$('#donationPopup button').click(function(){
			...
			if(reply == 'show'){
				window.open('https://anydice.com/donation', '_blank');
			...
			for(var j = 0, l = receivedDistributions.data.length; j < l; j++){
				...
				for(var iE = 0, lE = sourceData.length; iE < lE; iE++){
					...
					if(e[0] !== null && e[0] >= i && e[1] !== 0){
						...
						else if (data.length > 0 && data[data.length - 1] !== null) {
							data.push([null, null]);
						}
						break;
					}
	...
	// Trigger calculation with Enter + (Meta or Ctrl) key.
	document.body.addEventListener('keydown', (event) => {
	    if(
			event.key === "Enter" &&
			(event.metaKey || event.ctrlKey) &&
			event.target.id == "codeInput"
		) {
